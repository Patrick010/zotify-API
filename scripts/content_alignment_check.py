...
# Documents to scan for references
SCAN_SCOPE = [
...
# Key design documents for alignment checking
DESIGN_DOCS = {
...
def load_registered_files():
    """Load all non-exempt, registered files from TRACE_INDEX.yml."""
    ...
    for item in data:
        path = item.get('path')
        if item.get('type') != 'exempt' and item.get('registered') is True and path:
...
def build_cross_reference_map(all_registered_files):
    """
    Scans configured documents and builds a map of which file references which.
    """
    print("Building reference graph...")
    cross_ref_map = defaultdict(set)
    link_count = 0

    # Create a regex pattern to find any of the registered filenames
    # This is more efficient than iterating through every file for every other file
    # We look for the base name (e.g., 'AGENTS.md') and the full path
    ...
    # Remove duplicates and sort by length to match longer paths first
    ...
    for doc_path_str in SCAN_SCOPE:
        ...
        # Find all mentions of other files
        mentions = master_pattern.findall(content)

        for mention in set(mentions):
            # Resolve which full path the mention corresponds to
            # (e.g., if mention is 'linter.py', resolve to 'scripts/linter.py')
...
def validate_alignment(registered_files, cross_ref_map):
    """
    Validates each file for alignment and returns a list of results.
    """
    print("Validating file alignment...")
    results = []

    # Create a flattened set of all files that are referenced at least once
    ...
    for path, data in registered_files.items():
        ...
        if not linked_in_design: notes.append("Missing Design link (HLD/LLD)")
        if not linked_in_trace: notes.append("Missing Trace link (Alignment Matrix)")
        ...
        # Find where it is linked from
        linked_from = [source for source, targets in cross_ref_map.items() if path in targets]
...
def generate_report(results):
    """Generates and writes the final Markdown report."""
    ...
    coverage = int((aligned + partial) / total * 100) if total > 0 else 100
...
def main():
    parser = argparse.ArgumentParser(description="Content-Level Item Alignment Check")
    parser.add_argument("--scan", action="store_true", help="Scan and generate the alignment report (default action).")
    ...
    if not args.scan and not args.enforce:
        args.scan = True # Default to scan if no flags are given
    ...
    if args.enforce:
        print("\n--- Enforcing Policy ---")
        orphan_threshold = int(len(registered_files) * 0.1) # Allow max 10% orphans
        coverage_threshold = 90 # Require 90% coverage

        if coverage < coverage_threshold or orphans > orphan_threshold:
            print(f"❌ Content alignment check failed.")
            print(f"   - Coverage: {coverage}% (Threshold: {coverage_threshold}%)")
            print(f"   - Orphans: {orphans} (Threshold: {orphan_threshold})")
            sys.exit(1)
        else:
            print("✅ Content alignment coverage acceptable.")
            sys.exit(0)
