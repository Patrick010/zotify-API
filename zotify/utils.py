...
from zotify.const import (ALBUM, ALBUMARTIST, ARTIST, ARTWORK, COMPILATION,
                          DISCNUMBER, EXT_MAP, GENRE, LYRICS, M4A_TRACKID,
                          MP3_TRACKID, TOTALDISCS, TOTALTRACKS, TRACKNUMBER,
                          TRACKTITLE, YEAR)
...
def fill_output_template(
    ...
    output_template = output_template.replace(
        "{album_artist}", fix_filename(album_artists[0])
    )
    ...
    output_template = output_template.replace(
        "{total_tracks}", fix_filename(total_tracks)
    )
    output_template = output_template.replace("{id}", fix_filename(scraped_track_id))
    output_template = output_template.replace(
        "{track_id}", fix_filename(scraped_track_id)
    )
...
def regex_input_for_urls(
    ...
    base_url = (
        r"^(?:https?://)?open\.sp"
        + r"otify\.com(?:/intl-\w+)?/%s/([0-9a-zA-Z]{22})(?:\?si=.+?)?$"
    )
    if non_global:
...
# Metadata Utils
def conv_artist_format(
    artists: list[str], FORCE_NO_LIST: bool = False
) -> list[str] | str:
...
def set_audio_tags(
    ...
    if ext == "mp3":
        ...
        freeform_set(
            tags, M4A_TRACKID, type("tag", (object,), {"values": [scraped_track_id]})()
        )
    else:
        tags.tag_map["trackid"] = TAG_MAP_ENTRY(
            getter="trackid", setter="trackid", type=str
        )
...
def compare_audio_tags(
    ...
    for i in range(len(reliable_tags)):
        if isinstance(reliable_tags[i], list) and isinstance(
            reliable_tags_onfile[i], list
        ):
    ...
    for i in range(len(unreliable_tags)):
        if isinstance(unreliable_tags[i], list) and isinstance(
            unreliable_tags_onfile[i], list
        ):
...
def set_music_thumbnail(track_path: PurePath, image_url: str, mode: str) -> None:
    ...
    jpg_filename = (
        "cover.jpg"
        if "{album}" in Zotify.CONFIG.get_output(mode)
        else track_path.stem + ".jpg"
    )
...
def add_to_song_archive(
    track_id: str, filename: str, author_name: str, track_name: str
) -> None:
