...
from zotify.const import (DURATION_MS, EPISODE_URL, ERROR, ID, ITEMS, NAME,
                          PARTNER_URL, PERSISTED_QUERY, SHOW, SHOW_URL)
from zotify.termoutput import Loader, PrintChannel, Printer
from zotify.utils import (create_download_directory, fix_filename,
                          fmt_duration, wait_between_downloads)
    ...
    if podcast_name is None or episode_name is None or duration_ms is None:
        ...
        with Loader(PrintChannel.PROGRESS_INFO, "Preparing download..."):
            ...
            download_directory = PurePath(
                Zotify.CONFIG.get_root_podcast_path()
            ).joinpath(extra_paths)
            ...
            if (
                ...
                if stream is None:
                    ...
                    Printer.hashtaged(
                        ...
                        f'DOWNLOADED: "{Path(filename).relative_to(Zotify.CONFIG.get_root_podcast_path())}"\n'
