...
from zotify.const import (ID, ITEMS, NAME, PLAYLIST_URL, TRACK, TRACKS, TYPE,
                          USER_PLAYLISTS_URL)
...
from zotify.utils import (fill_output_template, split_sanitize_intrange,
                          strptime_utc)
...
def get_playlist_songs(playlist_id: str) -> tuple[list[str], list[dict]]:
    ...
    playlist_tracks = [
        (
            track_dict[TRACK]
            if track_dict[TRACK] is not None and track_dict[TRACK][ID]
            else None
        )
            ...
            try:
                if len(playlist_tracks) > 0:
                    ...
                    first_track_path, _ = fill_output_template(
                        output_template,
                        parse_track_metadata(playlist_tracks[0]),
                        extra_keys,
                    )
                ...
                if len(playlist_tracks) > 1:
                    ...
                    second_track_path, _ = fill_output_template(
                        output_template,
                        parse_track_metadata(playlist_tracks[1]),
                        extra_keys,
                    )
                    if (
                        PurePath(first_track_path).parent
                        != PurePath(second_track_path).parent
                    ):
                        raise ValueError(
