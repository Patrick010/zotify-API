...
- **Quality Assurance:**  
  ...
  - **Automated Documentation Workflow:** The project enforces its "living documentation" policy through an automated workflow. This includes a `pre-commit` hook (`lint-docs.py`) that requires documentation to be updated in the same commit as the code it describes, and a utility (`log-work.py`) to standardize the logging of all work. This workflow is a mandatory quality gate for all contributions. See the `automated_documentation_workflow.md` feature spec for details.
