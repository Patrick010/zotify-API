...
@router.post("", response_model=StandardResponse[Notification], dependencies=[Depends(require_admin_api_key)])
def create_notification(
    payload: NotificationCreate,
    notifications_service: NotificationsService = Depends(get_notifications_service),
):
    notification = notifications_service.create_notification(payload.user_id, payload.message)
