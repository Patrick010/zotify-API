...
def test_create_notification(mock_user_service: MagicMock) -> None:
...
def test_get_notifications(mock_user_service: MagicMock) -> None:
    service = NotificationsService(user_service=mock_user_service)
    service.get_notifications("user1")
    mock_user_service.get_notifications.assert_called_once_with("user1")


def test_mark_notification_as_read(mock_user_service: MagicMock) -> None:
    service = NotificationsService(user_service=mock_user_service)
