...
def test_reload_logging_config_success(mock_file, mock_get_service, monkeypatch):
    monkeypatch.setattr("zotify_api.config.settings.admin_api_key", "test_key")
    mock_service = MagicMock()
    mock_get_service.return_value = mock_service

    response = client.post("/api/system/logging/reload", headers={"X-API-Key": "test_key"})
...
@patch("builtins.open")
def test_reload_logging_config_file_not_found(mock_file, monkeypatch):
    monkeypatch.setattr("zotify_api.config.settings.admin_api_key", "test_key")
...
def test_reload_logging_config_yaml_error(mock_file, monkeypatch):
    monkeypatch.setattr("zotify_api.config.settings.admin_api_key", "test_key")
    response = client.post("/api/system/logging/reload", headers={"X-API-Key": "test_key"})
    assert response.status_code == 400

@patch("builtins.open", new_callable=mock_open, read_data="logging:\n  default_level: 123\n  sinks: []")
def test_reload_logging_config_validation_error(mock_file, monkeypatch):
    monkeypatch.setattr("zotify_api.config.settings.admin_api_key", "test_key")
    response = client.post("/api/system/logging/reload", headers={"X-API-Key": "test_key"})
